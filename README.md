...
- No Firefox, a barra é lida logo após o evento e depois a cada 120 ms; a URL conta quando o mesmo domínio aparece em duas leituras seguidas. Na primeira leitura de um site bloqueado, a cortina já cobre a tela enquanto a URL é confirmada (se a leitura seguinte mostrar outro domínio, ela sai; no máximo 1,5 segundo). Confirmado o site, a troca começa sem esperar o desenho da cortina, que já está na tela.
...
- **Nenhuma família se encaixa:** o navegador volta para a tela inicial (ação **Início**), com um aviso, e fica registrado como rejeitado. O primeiro bloqueio espera 2 segundos e um novo teste, para não bloquear um navegador que mostra a URL na barra depois do conteúdo. O pacote fica salvo com a versão instalada (a data da última atualização do navegador e a do app). Nessa versão, ele é fechado assim que a janela dele aparece, sem esperar uma página e sem testar as famílias de novo. Depois de uma atualização do navegador ou do app, as famílias são testadas de novo, para o caso de a atualização passar a funcionar: ele é fechado assim que mostra uma página, a menos que uma família tenha acabado de ler a URL, e a recusa passa a valer para a nova versão.
