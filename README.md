...
Via (`mark.via.gp`, `mark.via`), UC Browser (`com.UCMobile.intl`, `com.UCMobile`) e UC Mini (`com.uc.browser.en`) ficam em `KNOWN_UNSUPPORTED`: nos testes, a barra deles mostrava o título da página, e sites abertos por links ou pela pesquisa ficavam acessíveis. Eles nunca são identificados e são fechados no primeiro evento da janela deles, enquanto houver sites na lista ou o bloqueio de pornografia estiver ligado: sem esperar uma página, sem filtrar pelo tipo de evento e sem percorrer a árvore (só a janela ativa é consultada, para uma notificação ou uma janela fora de uso não fechar o app em uso).
...
- **Nenhuma família se encaixa:** o navegador volta para a tela inicial (ação **Início**), com um aviso, e fica registrado como rejeitado na versão instalada (a data da última atualização dele e a do app, como na rede de segurança abaixo). O primeiro bloqueio espera 1 segundo e um novo teste, para não bloquear um navegador que mostra a URL na barra depois do conteúdo; se uma família leu a URL, o teste se repete a cada 0,4 segundo até a confirmação. Na mesma versão, um navegador rejeitado é fechado no primeiro evento da janela dele, como o Via e o UC, sem esperar uma página. Depois de uma atualização dele ou do app, é testado de novo: fechado assim que mostra uma página, a menos que uma família tenha acabado de ler a URL.
- **Fechamento repetido:** logo depois da ação **Início**, os eventos do navegador que ainda está saindo da tela são ignorados por 0,4 segundo. Se ele for reaberto e estiver na janela ativa, é fechado de novo na hora, sem repetir o aviso.
